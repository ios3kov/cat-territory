...
export function DailyTerritory({ onClose, onAchievements }: Props) {
  ...
  const [board, setBoard] = useBoardState(saved?.board ?? pristine),
    ...
    [summary, setSummary] = useState<Summary | null>(() => {
      ...
      return {
        label: getFinishLabel(getFinishGrade(r.mistakes, r.usedHint)),
  ...
  const mistakesRef = useRef(mistakes),
    ...
    startedRef = useRef(started),
    startedAt = useRef(Date.now() - seconds * 1000),
    ...
    restartArmTimer = useRef<number | null>(null),
  ...
  const reset = () => {
    ...
    setMistakeNotice(null);
    if (restartArmTimer.current !== null) clearTimeout(restartArmTimer.current);
  ...
  const requestRestart = () => {
    ...
    if (!restartArmed) {
      ...
      if (restartArmTimer.current !== null)
        clearTimeout(restartArmTimer.current);
      restartArmTimer.current = window.setTimeout(
        () => setRestartArmed(false),
        2200,
      );
      return;
  ...
  useEffect(() => {
    if (!started || won || celebrating) return;
    const update = () => {
  ...
  useEffect(() => {
    ...
    completionTimer.current = window.setTimeout(
      () => {
        ...
        haptic('win');
        setSummary({
          label: getFinishLabel(getFinishGrade(mistakes, usedHintRef.current)),
